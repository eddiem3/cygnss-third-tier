     ...
     1 iyinc,ibsteps,ibinc,iasteps,iainc,iastart          
     ...
     3 data,rmax,Amax,Bmax,xmax,ymax,wndspeed2
      itest1=0
      write(*,*)
      write(*,*)'This program is for bingen created files'
      write(*,*)
      write(*,*)'This program reads DDM files generated by bingen.for'
...
* Read the metadata header first
      read(9)windsteps,windstart,windinc,ixsteps,ixinc,iysteps,iyinc,
     1ibsteps,ibinc,iasteps,iainc,iastart           
*10    read(10,*,end=1000) i,j,k,l,m,wndspeed,x,y,A,B,Pdelt,
*     1 (convary(kk),kk=1,600)
10    read(9,end=1000) i,j,k,l,m,wndspeed,x,y,A,B,Pdelt,
     1 (convary(kk),kk=1,600)
      itot=itot+1
...
*
*********************************************************************
*Modification to use the values brought in from bingen.for
**********************************************************************
      rmax=0
*      do k=1,11
*      do ll=1,21
...
* The location in the synthetic waveform array is calculated not input now
      N=int(y/10000)
      M=int(x/10000)
      i=1+N*ibsteps*iasteps+M*iasteps*ibsteps*ixsteps+ibsteps*iasteps*
     1 ixsteps*(k-1)+(mm-1)+(ll-1)*ibsteps
...
*      read(*,*)j      
      ...
      crosswork=0
*************************************************
* Beginning of write-out section
...
*      Write(*,*)
* End of write-out section      
***************************************************      
* For condition of normal operation, get unknown storm data as usual      
*
*********************
* Section to write out pieces of data being processed
