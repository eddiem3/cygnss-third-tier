      subroutine trackconvolve(height,tanumax,numslops,slopary,
...
*
* Change the scale factor to the slopary if this section is entered, otherwise leave at 1.0
*
	   
      scale=scale2
	index=1+int(sqrt(2*CTcBYn1*(i2)/height)*100/tanumax)
	index2=1+int(sqrt(2*CTcBYn1*(i2+1)/height)*100/tanumax)

      do ii=index,100
      if(ii.ge.index2) goto 120
      sum=sum+ylambd2*tanumax*(ii-1)*(slopary(ii))/100	   
*      sum=sum+tanumax*(ii-1)*(slopary(ii))/100	   
      enddo	
*121   write(*,*)'indx',index,' indx2',index2,' tanumx',tanumax,' tx0'
*     1	,tanx0,' tanx',tanx,' sum= ',sum*tanumax/100
*      pause 
*      goto 120 
	   else
*
